...
| L | Digital DNA + NCD: pre-flight and P4 core (H3) | D, E | L | `done (2026-08-24) — kill criterion FIRED: 162 bots (4.83%) survive ≥100 events vs required 335 → H3 CANCELLED, survivor table published; D7 all viable; R10 blocked (acss name collision); P9/P12 added, checks 12/12; bitacora 18` |
| M | Ledger, findings document, hand-off quality (recurring) | – | S | `done (2026-08-25) — final pass with WP-N: three-random-numbers audit elementwise across bitacoras 17–19; FINDINGS F15 closes the charter criteria; HANDOFF current; bitacora 19` |
| N | **Protocol C execution — H4, the primary claim** | B, E, F, G, H, I | L | `done (2026-08-25) — SCOPED by target-modality audit: no spectral/text family computable on the obtainable TB20 artefact → H4 UNTESTABLE_PENDING_DATA; metadata side measured (+0.3143 ± 0.0075 naive hgb, R8 clean both variants); G4 null 0.0088; bitacora 19` |
...
| exec-note | 2026-08-24 | WP-L execution resolutions (bitacora 18 §7): the kill-criterion cancellation is the recorded outcome of the G4 = H3 gate (acceptance box reads "executed the cancellation"); R10 recorded BLOCKED on the acss PyPI name collision rather than substituted; P9 asserts the R6 intent with the plan's "BDM ≪ its block entropy" shorthand recorded as not holding under the standard definition; pybdm's block-shape semantics and CTM coverage measured and documented. |
| exec-note | 2026-08-25 | WP-N execution resolutions (bitacora 19): the target-modality audit (§0 ambiguity protocol step 4) scoped "every family available" to META_aligned(4) + VOL_PROFILE(1) — tweets_tensor.pt is dense pooled embeddings (non-zero fraction 1.000000 measured), raw TwiBot-20 gated by its authors, open TwiBot-22 user.json profiles-only; H4 recorded UNTESTABLE_PENDING_DATA rather than re-scoped or target-swapped; R8 recal variant defined as the WP-B diagnostic (StandardScaler on labelled target columns) after the corpus-reconstruction route proved a provable no-op; G4 null uses WP-K permutation halves because cresci_meta_v1.npz carries no ids ("account id hash" intent preserved, deviation recorded); padded-arm noise drawn once per side per arm from default_rng(RNG_DRAWS·1000 + salt), printed. |
| exec-note | 2026-08-25 | H4-T amendment (bitacora 23, post-plan, registered before execution): target-side modality re-audit under the raw release showed the temporal families uncomputable by benchmark construction; scope narrowed namedly to SPEC_X vs META_raw/VOL/TOKENS/SHAN floors; equal-cap window control registered for the target's 200-text cap; provenance = owner decision, Kaggle copy labelled PRELIMINARY with sha256 chain and a re-derivation obligation on the authorized copy. VERDICT: H4-T failed — text spectra degrade more than metadata (F16). |
| exec-note | 2026-08-25 | LS-1 amendment (bitacora 24, registered before any language computation, executed same day): tooling verified before registration (langid 1.1.6 installable; lingua-py unresolvable — recorded, R10-style); per-tweet secondary ran on EVERY tweet since measured cost made the unspecified subsample knob unnecessary; fidelity gates vs the H4-T artefact exact (0.0) on eight arms and on the full-pipeline null. VERDICT: PARTIAL-EFFECT — language shift real (P1 PASS: Cresci humans 5.67 % EN vs TB20 both classes 75.6–93.1 % EN), mediation REFUTED (Δ_INV −0.0028, 0/20; local separability worsens under matching); fingerprint failure is corpus-lineage-bound, not language-tag-bound (F17). Caveat: restricted source holds 109 humans. |
